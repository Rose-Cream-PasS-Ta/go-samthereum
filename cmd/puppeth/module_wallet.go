...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'g3th --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec g3th --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
