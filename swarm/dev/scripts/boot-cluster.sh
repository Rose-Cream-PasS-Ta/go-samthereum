...
#
# The cluster contains a bootnode, a g3th node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the g3th and
# swarm nodes
start_bootnode() {
...
# start_g3th_node starts a gcm node with --datadir pointing at <base-dir>/g3th
# and a single, unlocked account with password "g3th"
start_g3th_node() {
  local dir="${base_dir}/g3th"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get g3th account address"
  fi
...
create_account() {
  ...
  g3th --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
