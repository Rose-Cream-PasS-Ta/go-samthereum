...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	// Send the proof request and verify the response
	cost := server.peer.peer.GetRequestCost(g3thelperTrieProofsMsg, len(requestsV2))
	sendRequest(server.peer.app, g3thelperTrieProofsMsg, 42, cost, requestsV2)
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		// Send the proof request and verify the response
		cost := server.peer.peer.GetRequestCost(g3thelperTrieProofsMsg, len(requests))
		sendRequest(server.peer.app, g3thelperTrieProofsMsg, 42, cost, requests)
