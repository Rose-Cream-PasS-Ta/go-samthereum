...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	if full {
		...
		hash1 = blockchain.g3theaderByNumber(uint64(i)).Hash()
		hash2 = blockchain2.g3theaderByNumber(uint64(i)).Hash()
	}
	if hash1 != hash2 {
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	if full {
		...
		for header := blockchain.g3theaderByNumber(blockchain.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, blockchain.g3theaderByNumber(header.Number.Uint64()-1) {
	...
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		for j := 0; j < i-failAt; j++ {
			if full {
				...
				if header := blockchain.g3theaderByNumber(failNum + uint64(j)); header != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	for i := 0; i < len(blocks); i++ {
		...
		if fheader, aheader := fast.g3theaderByHash(hash), archive.g3theaderByHash(hash); fheader.Hash() != aheader.Hash() {
			t.Errorf("block #%d [%x]: header mismatch: fastdb %v, archivedb %v", num, hash, fheader, aheader)
		}
		if anheader, arheader := ancient.g3theaderByHash(hash), archive.g3theaderByHash(hash); anheader.Hash() != arheader.Hash() {
...
func TestLowDiffLongChain(t *testing.T) {
	...
	for number := head.NumberU64(); number > 0; number-- {
		if hash := chain.g3theaderByNumber(number).Hash(); hash != header.Hash() {
			t.Fatalf("header %d: canonical hash mismatch: have %x, want %x", number, hash, header.Hash())
		}
		header = chain.g3theader(header.ParentHash, number-1)
	}
}
...
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	...
	if headerByNum := chain.g3theaderByNumber(canonNum); headerByNum != nil {
