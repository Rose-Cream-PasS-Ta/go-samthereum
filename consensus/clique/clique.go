...
func (c *Clique) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 || (number%c.config.Epoch == 0 && (len(headers) > params.ImmutabilityThreshold || chain.g3theaderByNumber(number-1) == nil)) {
			checkpoint := chain.g3theaderByNumber(number)
			if checkpoint != nil {
		...
		if len(parents) > 0 {
			...
			header = chain.g3theader(hash, number)
			if header == nil {
				return nil, consensus.ErrUnknownAncestor
			}
		}
